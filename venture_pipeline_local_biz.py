#!/usr/bin/env python3
"""
LOCAL_BIZ Venture Pipeline — Executes ALL LOCAL_BIZ-type DAG configs.
...
This ONE script replaces 2 individual stub runners.

Cron: 0 8 * * * (daily 8 AM, after morning intelligence)

Usage:
    python3 AUTOMATIONS/venture_pipeline_local_biz.py --run
    python3 AUTOMATIONS/venture_pipeline_local_biz.py --run --limit 10
    python3 AUTOMATIONS/venture_pipeline_local_biz.py --status
    python3 AUTOMATIONS/venture_pipeline_local_biz.py --dry-run
"""
...
LOG_FILE = AUTOMATIONS / "logs" / "venture_pipeline_local_biz.log"
...
def log(msg: str, level: str = "INFO") -> None:
    ...
    line = f"[{ts}] [LOCAL_BI-PIPE] [{level}] {msg}"
    print(line)
...
def load_content_dags() -> list[dict]:
    ...
    for f in sorted(DAG_DIR.glob("dag_*.json")):
        try:
            ...
            if d.get("venture", "").upper() == "LOCAL_BIZ":
                d["_file"] = f.name
                dags.append(d)
        except Exception:
            continue
...
def execute_dag(dag: dict, dry_run: bool = False) -> dict:
    ...
    if not dry_run:
        capture_skill_from_result(
            task=f"local_biz pipeline: {method[:60]}",
...
def run(limit: int = 0, dry_run: bool = False) -> None:
    ...
    log(f"Starting local_biz pipeline: {len(dags)} DAGs to execute")
...
def status() -> None:
    dags = load_content_dags()
    print(f"LOCAL_BIZ Pipeline Status")
...
def main() -> None:
    parser = argparse.ArgumentParser(description="LOCAL_BIZ venture pipeline")
